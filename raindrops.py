...
import pygame
from rain_drop_settings import Settings
from raindrop_image import Image
...
class Raindrops:
    ...
    def _create_row(self, row_number):
        for drop_number in range(self.number_drops_x):
            self._create_single_drop(drop_number, row_number)

    def _create_single_drop(self, drop_number, row_number):
        drop = Image(self)
        drop_width, drop_height = drop.rect.size
